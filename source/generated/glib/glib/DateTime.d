...
public class DateTime
{
	...
	/**
	 * Creates new DateTime with the current time
	 * Params:
	 *     utc = Utc or local
	...
	public this(bool utc = true) {
		...
		if (utc) p = g_date_time_new_now_utc();
		else p = g_date_time_new_now_local();

		if(p is null) {
			throw new ConstructionException("null returned by g_date_time_new_from_now_local/utc");
	}
 ...
 * zone returned by g_time_zone_new_utc/local().
 *
 * Params:
 ...
 *     utc = For changing between utc and local
 *
 * Returns: a #GDateTime, or %NULL
 *
 * Since: 2.26
 ...
 * Throws: ConstructionException GTK+ fails to create the object.
 */
public this(int year, int month, int day, int hour, int minute, double seconds, bool utc = true) {
	GDateTime* __p;

	if (utc) __p = g_date_time_new_utc(year, month, day, hour, minute, seconds);
	else __p = g_date_time_new_local(year, month, day, hour, minute, seconds);
...
public this (long t, bool utc = true) {
	...
	if ( utc ) p = g_date_time_new_from_unix_utc(t);
	else p = g_date_time_new_from_unix_local(t);
...
public this (ref GTimeVal tv, bool utc = true) {
	...
	if ( utc ) p = g_date_time_new_from_timeval_utc(&tv);
	else p = g_date_time_new_from_timeval_local(&tv);
...
/**
 * Translate the object to the hash
 * Return: Hash of the DateTime
 */
