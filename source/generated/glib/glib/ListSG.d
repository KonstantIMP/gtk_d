...
public class ListSG
{
	...
	/**
	 * Getter for SLists data
	 * Returns: Data of the Slist
	 */
	@property void* data() {
		return gSList.data;
	}
 ...
 */
public T[] toArray(T, TC = getCType!T)() if ( is(T == class) ) {
	T[] arr = new T[length()];
	ListSG list = this;
	size_t count;
...
/** Ditto */
public T[] toArray(T)() if ( is ( T == string ) ) {
	T[] arr = new T[length()];
	ListSG list = this;
	size_t count;
...
/**
 * Getter for base C-type
 * Params:
 *     T = Type for c-type getting
 * Returns: Ptr to the C type or the void*
 */
